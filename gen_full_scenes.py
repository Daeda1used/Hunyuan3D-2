...
if __name__ == '__main__':
    ...
    # 使用多进程处理
    num_processes = min(cpu_count()//16, len(scene_folders))  # 限制并发数为 CPU 核心数或场景数
    print(f"Using {num_processes} processes.")

    with Pool(num_processes) as pool:
        pool.starmap(process_scene, [(scene_folder, base_dataset_folder) for scene_folder in scene_folders])
